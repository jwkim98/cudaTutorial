...
namespace cuda_lib{
    ...
    __global__ void matAdd(int M, int N, data_type *A, data_type *B, data_type *C){
        ...
        int col_index = blockDim.x*blockIdx.x + threadIdx.x;//index to calculate from whole grid
        int grid_size = gridDim.x*gridDim.y
        int index = gridDim.x*row + col;

        for(int row = row_index; row < M; row += gridDim.y){
            for(int col = col_index; col < N; col += gridDim.x){ // in case of matrix size exceeding grid size
                if(row<M && col<N){
    ...
    void cudaMatrix::MatMulAsync(int N, float *inA, float *inB, float *outC, cudaStream_t stream){
           ...
           //Start kernel
           matMul<<<numBlocks ,threadsPerBlock ,0 ,stream_0>>>(N, g_inA, g_inB, outC);
